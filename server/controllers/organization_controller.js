	...
	if(daughters)  {
		for (let index = 0, length = daughters.length; index < length; index++) {
			for (let j = index + 1; j < length; j++) {
				script += insertSiblingRelation(daughters[index]['org_name'], daughters[j]['org_name']);
			}
...
function insertParentChildRelation(parentName, childName) {
	return `INSERT INTO RELATIONSHIPS(START_ORG, END_ORG, RELATIONSHIP_TYPE)
	 ...
	 FROM(SELECT id FROM organizations WHERE LOWER(name) = '${parentName.toLowerCase()}') p
	, (SELECT id FROM organizations WHERE LOWER(name) = '${childName.toLowerCase()}') c;`
...
function insertSiblingRelation(daughter1, daughter2) {
	return `INSERT INTO RELATIONSHIPS(START_ORG, END_ORG, RELATIONSHIP_TYPE)
	 ...
	 FROM(SELECT id FROM organizations WHERE LOWER(name) = '${daughter1.toLowerCase()}') p
	, (SELECT id FROM organizations WHERE LOWER(name) = '${daughter2.toLowerCase()}') e;`
